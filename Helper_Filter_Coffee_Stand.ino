/*
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
PROJECT NAME   : Helper Filter Coffee Stand
...
Purpose        : This stand is designed for making the brewing process easier and drinking a well-brewed,non-burned filter coffee. The summarized process of this stand
               is measuring the hot water temperature, helping it to cool with using a cooler fan, pulling the filter coffee into the french press, and giving an alert 
...
OneWire oneWire(DS18B20_sensor);        //to define DS18B20_sensor as one wire communication interface
DallasTemperature DS18B20(&oneWire);    //to define the defined one wire sensor as DS18B20 thermometer 
...
void loop() 
{
  ...
  for(i=0;i<=20;i++){                      //take the data into a loop to give some time to come exact heat
    ...
    delay(500);                            //to wait for 500 millisecond 
    }
  
  do{
      DS18B20.requestTemperatures();       //to request the temperature sensor to measure temperature
      Celsius=DS18B20.getTempCByIndex(0);  //to measure the temperature as celcius       
      Serial.print(Celsius);               //printing the sensor data to check the data on serial monitor
      Serial.print(" Celsius\n");          //add "celisius" to end of the data
      delay(10);                           //to wait for 10 millisecond 
   }while(Celsius>=87);
  ...
  for(i=0;i<=5;i++){                      //to give an alert sound 5 times to indicate process ending
     tone(buzzer,1600);                    //adjusting the sound of buzzer to 800 Hz
     delay(400);                          //to wait for 400 milliseconds
     noTone(buzzer);                      //to silence the buzzer
     delay(200);                          //to wait for 200 milliseconds
     
     tone(buzzer,1400);                    //adjusting the sound of buzzer to 600 Hz
     delay(400);                          //to wait for 400 milliseconds
     noTone(buzzer);                      //to silence the buzzer
     delay(200);                          //to wait for 200 milliseconds
   }
